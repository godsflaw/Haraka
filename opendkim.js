...
// There is one OpenDKIMObject created for each signature found

function OpenDKIMObject (header, header_idx, cb, timeout) {
    this.cb = cb;
    this.sig = header;
...
OpenDKIMObject.prototype.debug = function (str) {
    util.debug(str);
};

OpenDKIMObject.prototype.header_canon_relaxed = function (header) {
    var m = /^([^:]+):\s*((?:.|[\r\n])*)$/.exec(header);
    if (!m) return header;
...
OpenDKIMObject.prototype.add_body_line = function (line) {
...
OpenDKIMObject.prototype.result = function (error, result) {
    this.run_cb = true;
    return this.cb(
...
OpenDKIMObject.prototype.end = function () {
    if (this.run_cb) return;
...
exports.OpenDKIMObject = OpenDKIMObject;

//////////////////////////
// OpenDKIMVerifyStream //
//////////////////////////

function OpenDKIMVerifyStream(cb, timeout) {
    Stream.call(this);
    this.run_cb = false;
    var self = this;
...
OpenDKIMVerifyStream.prototype.handle_buf = function (buf) {
    var self = this;
            ...
            if (!this._in_body) {
                ...
                if (!this.header_idx['dkim-signature']) {
                    ...
                    for (var d=0; d<dkim_headers.length; d++) {
                        this.dkim_objects.push(new OpenDKIMObject(dkim_headers[d], this.header_idx, callback, this.timeout));
                    }
                    if (this.pending === 0) {
...
exports.OpenDKIMVerifyStream = OpenDKIMVerifyStream;
